...
class TestMemoryGameLogic(unittest.TestCase):
    ...
    def test_game_completion_logic(self):
        ...
        def simulate_check_completion(card_states, moves_count):
            ...
            all_found = all(
                state["found_pair"] for state in card_states.values()
            )
            if all_found:
                return "win"
    ...
    def test_process_match_logic(self):
        ...
        new_moves, is_match = simulate_process_match(
            match_result=False, current_moves=10
        )
        self.assertEqual(
            new_moves, 9, "При несовпадении ходы должны уменьшиться"
        )
        self.assertFalse(is_match, "Должно быть False при несовпадении")

        new_moves, is_match = simulate_process_match(
            match_result=True, current_moves=10
        )
    ...
    def test_flip_card_logic(self):
        ...
        self.assertTrue(
            result, "При передаче изображения карточка должна быть перевернута"
        )
    ...
    def test_update_card_state_logic(self):
        ...
        new_state = update_card_state(
            original_state, is_match=True, was_turned=True
        )

        self.assertTrue(
            new_state["found_pair"], "Карточка должна быть найдена"
        )
        self.assertFalse(new_state["turned_over"], "Карточка должна скрыться")

        original_state = {"found_pair": False, "turned_over": True}
        new_state = update_card_state(
            original_state, is_match=False, was_turned=True
        )

        self.assertFalse(
            new_state["found_pair"], "Карточка не должна быть найдена"
        )
        self.assertFalse(new_state["turned_over"], "Карточка должна скрыться")
    ...
    def test_reset_turned_cards_logic(self):
        ...
        self.assertFalse(
            card_states[2]["turned_over"], "Карточка 2 должна скрыться"
        )
        self.assertFalse(
            card_states[3]["turned_over"], "Карточка 3 должна скрыться"
        )

    def test_full_turn_logic(self):
        """Тест 8: Полная логика одного хода."""

        def simulate_complete_turn(
            card_idx, current_states, current_turned, values
        ):
            """Симуляция полного хода."""
            if (
                current_states[card_idx]["found_pair"]
                or current_states[card_idx]["turned_over"]
                or len(current_turned) >= 2
            ):
        ...
        self.assertTrue(
            updated_states[0]["found_pair"], "Карточка 0 найдена в паре"
        )
        self.assertTrue(
            updated_states[1]["found_pair"], "Карточка 1 найдена в паре"
        )
