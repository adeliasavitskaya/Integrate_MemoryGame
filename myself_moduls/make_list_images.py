...
def list_files(dir_paths):
    """Находит изображения в указанных директориях и подготавливает пары для игры.

    Требуется минимум 8 уникальных изображений для создания 8 пар (16 карточек).

    Args:
        dir_paths: Кортеж путей к директориям для поиска изображений.
    ...
    for dir_path in dir_paths:
        ...
        try:
            ...
            continue
    if len(all_images) < 8:
        raise FileNotFoundError(f"Недостаточно изображений для игры в: {dir_paths}")
